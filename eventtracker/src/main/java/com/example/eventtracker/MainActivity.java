...
public class MainActivity extends AppCompatActivity implements Adapter.OnLongClickListener {
    ...
    List<EventModel> pinnedNotesList;
    List<EventModel> unpinnedNotesList;
    FloatingActionButton fab_add;
    EventModel selectedNote;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        EventDatabase notesDatabase = new EventDatabase(this);
        eventModelList = notesDatabase.getNote();

        pinnedNotesList = new ArrayList<>();
        unpinnedNotesList = new ArrayList<>();

        for (EventModel note : eventModelList) {
            if (note.isPinned()) {
        ...
        List<EventModel> combinedNotesList = new ArrayList<>();
        ...
        fab_add.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(MainActivity.this, event_add.class);
    ...
    private void performSearch(String query) {
        List<EventModel> filteredList = filterNotes(eventModelList, query);
        adapter.setNotes(filteredList);
        adapter.notifyDataSetChanged();
    }

    private List<EventModel> filterNotes(List<EventModel> notes, String query) {
        List<EventModel> filteredList = new ArrayList<>();

        for (EventModel note : notes) {
            if (note.getEventTitle().toLowerCase().contains(query.toLowerCase()) ||
                    note.getEventDetails().toLowerCase().contains(query.toLowerCase())) {
    ...
    @Override
    public void onLongClick(EventModel noteModel) {
        selectedNote = noteModel;
        showPopup(selectedNote);
    }

    private void showPopup(EventModel noteModel) {
                ...
                public boolean onMenuItemClick(MenuItem item) {
                    ...
                    if (item.getItemId() == R.id.pin) {
                        ...
                        if (selectedNote.isPinned()) {
                            Toast.makeText(MainActivity.this, "Event Pinned", Toast.LENGTH_SHORT).show();
                            pinnedNotesList.add(selectedNote);
                            unpinnedNotesList.remove(selectedNote);
                        } else {
                            Toast.makeText(MainActivity.this, "Event Unpinned", Toast.LENGTH_SHORT).show();

                            int previousPosition = eventModelList.indexOf(selectedNote);
    ...
    private void refreshRecyclerView() {
        List<EventModel> combinedList = new ArrayList<>();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_UPDATE_NOTE && resultCode == RESULT_OK) {
            if (data != null && data.hasExtra("UpdatedNote")) {
                EventModel updatedNote = data.getParcelableExtra("UpdatedNote");
                refreshActivity();
            }
        }
    }
